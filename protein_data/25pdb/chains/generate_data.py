...
outfile=sys.argv[3]
df=pandas.read_csv(labelfile)
mydict=df.set_index('id')['label'].to_dict()
...
for pdbid in pdblist:
		...
		label="_"
		if pdbid in mydict:
			label=mydict.get(pdbid)
		else :
			label=mydict.get(pdbid[:4])
		if label == None :
			print(pdbid,"label not found")
			label="_"
		s1="_"
		s2=s1
		s3=s2
		...
		with open(outfile,"a") as f:
				f.write(pdbid+'\t'+str(label)+'\t'+s1+'\t'+s2+'\t'+s3+'\n')
